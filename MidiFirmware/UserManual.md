...
( Exit minicom by typing CTRL+A Z Q <Enter> )
...
```
The event log 'J' command is useful to monitor MIDI messages being sent to the computer. However
using the event log may slightly slow down performance and so should not be enabled during normal use.

# MIDI Interface Menu

To monitor activity on attached devices type '?' to view the MIDI interface menu.
```
MIDI Interface Menu
...
Some TM1637 LED Display modules ship with excessively large value filter capacitors on Data and Clock inputs. The capacitors not only prevent the seven segment display updates, but also disrupt I2C LCD Display and button shift register operation. If fitted, these filter capacitors need to be either removed or significantly reduced in value.
...
```
Shift Register MIDI Channel 12 Note Numbers
...
SR 6   40   41   42   43   44   45   46   47  
SR 7   48   49   50   51   52   53   54   55  
SR 8   56   57   58   59   60   61   62   63  
...
Times in microseconds
  ...
  344902      999      63    7711  Loop
       ...
       0        0       0       0  Spare
```
It seems as though MIDI send execution times are longer when GrandOrgue is not connected.
